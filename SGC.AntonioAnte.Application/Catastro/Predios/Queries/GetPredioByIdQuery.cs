...
{
    public record GetPredioByIdQuery(Guid Id) : IRequest<PredioDto?>;

    public class GetPredioByIdQueryHandler : IRequestHandler<GetPredioByIdQuery, PredioDto?>
    {
        private readonly IApplicationDbContext _context;

        public GetPredioByIdQueryHandler(IApplicationDbContext context)
        {
            _context = context;
        }

        public async Task<PredioDto?> Handle(GetPredioByIdQuery request, CancellationToken cancellationToken)
        {
            var p = await _context.Predios
                ...
                .FirstOrDefaultAsync(x => x.Id == request.Id && x.EstadoActivo, cancellationToken);
